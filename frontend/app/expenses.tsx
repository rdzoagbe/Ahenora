...
import { ChevronLeft, Plus, Trash2, TrendingDown, TrendingUp } from 'lucide-react-native';

import { PressScale } from '../src/components/PressScale';
import { useUI, UIColors } from '../src/components/Kit';
import { useStore } from '../src/store';
import { api, Expense, ExpenseOverview, MerchantRow } from '../src/api';
...
/**
 * House expenses — what a household spends on the shopping, by month and by shop.
 *
 * Two rules from the server are repeated in the UI rather than hidden, because
 * they are the difference between a number that helps and a number that misleads:
 * every total shows how many receipts it is built from, and the month still
 ...
 * false. Eight seconds of typing is right every time; the scan can arrive once
 * the rest of this has proved itself.
 */
export default function HouseExpenses() {
  ...
  // Rendering nothing until the client has mounted keeps the served HTML and
          ...
          {adding ? (
            <View style={styles.card}>
              ...
              <PressScale testID="exp-save" onPress={save} disabled={saving} style={[styles.primary, saving && styles.dim]}>
                <Text style={styles.primaryText}>{saving ? t('saving') : t('exp_save')}</Text>
              </PressScale>
