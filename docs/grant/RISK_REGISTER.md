...
| T4 | Scale: unindexed queries and blocking work slow the service as households grow | L | M | 65 indexes with a guard test that fails on an unindexed query; password hashing, cropping and model discovery off the event loop; measured timings endpoint | Pool tuning before growth | Engineering |
